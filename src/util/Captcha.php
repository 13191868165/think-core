...
class Captcha
{
    ...
    //x轴因子
    protected $xFactor = 1.5;
    //x轴随机数范围
    protected $xRange = [1.2, 1.6];
    //y轴随机数范围
    protected $yRange = [10, 20];
    //旋转角度范围
    protected $angleRange = [-40, 40];
     ...
     * @param string $hash 用户验证码
     * @param bool $del 是否强制删除验证码
     * @return bool 用户验证码是否正确
     */
    public function check(string $code, string $hash, bool $del = false): bool
    {
    ...
    public function create(string $config = null): array
    {
        ...
        foreach ($text as $index => $char) {
            ...
            $x = $this->fontSize * ($index + 1) * mt_rand($this->xRange[0], $this->xRange[1]) * ($this->math ? 1 : $this->xFactor);
            $y = $this->fontSize + mt_rand($this->yRange[0], $this->yRange[1]);
            $angle = $this->math ? 0 : mt_rand($this->angleRange[0], $this->angleRange[1]);
